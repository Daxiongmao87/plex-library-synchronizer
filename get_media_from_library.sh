...
if [[ -z $PLEX_TOKEN ]];       then echo "Missing environment variable \$PLS_TOKEN"; exit; fi
if [[ -z $LIBRARIES_STRING ]]; then echo "Missing environment variable \$PLS_LIBRARIES"; exit; fi
if [[ -z $OUTPUTS_STRING ]];   then echo "Missing environment variable \$PLS_OUTPUTS"; exit; fi
if [[ -z $INTERVAL ]];         then echo "Missing environment variable \$PLS_INTERVAL, will run once."; fi
...
while true; do
  ...
  for j in $(seq 0 $(( ${#LIBRARIES[@]}-1 ))); do
    ...
    echo $(timestamp) "  - Direct URL: $CONNECTION"
